...
describe("o-squishy-list behaviour with More", () => {
	...
	describe("Items are re-shown when size allows", () => {
		...
		it("More element is hidden", () => {
			proclaim.lengthEquals(pcfEl.querySelectorAll('[data-more][aria-hidden="true"]'), 1);
		});
	...
	describe("getHiddenItems(), getRemainingItems()", () => {
		...
		it("When items without priority are hidden", () => {
			...
			proclaim.deepEqual(testPCF.getRemainingItems(), nodeListToArray(pcfEl.querySelectorAll(':not([aria-hidden="true"]):not([data-more])')));
		});
	...
	describe("Events fired when items are hidden or shown", () => {
		...
		it("When items without priority are hidden", () => {
			...
			proclaim.deepEqual(pcfLastEvent.remainingItems, nodeListToArray(pcfEl.querySelectorAll(':not([aria-hidden="true"]):not([data-more])')));
		});
	...
	describe("More item is shown when items are hidden", () => {
		...
		afterEach(() => {
			proclaim.lengthEquals(pcfEl.querySelectorAll('[data-more]:not([aria-hidden="true"])'), 1);
		});
